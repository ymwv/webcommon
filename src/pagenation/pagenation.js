...
pagenation.prototype = {
    ...
    preUrl: function() {
        ...
        var url = window.So360_Utils.urlAddQueryItem(`${this.baseUrl}&page=${pagenum}`, 'qc', 'utf8');
        return url;
    },
    firstUrl: function() {
        // eslint-disable-next-line
        if (this.useEvent) return 'javascript:void(0);';
        var url = window.So360_Utils.urlAddQueryItem(`${this.baseUrl}&page=1`, 'qc', 'utf8');
        return url;
    },
    countUrl: function() {
        // eslint-disable-next-line
        ...
        var url = window.So360_Utils.urlAddQueryItem(`${this.baseUrl}&page=${this.pageCount}`, 'qc', 'utf8');
        return url;
    },
    nextUrl: function() {
        // eslint-disable-next-line
    ...
    getCurrUrl(num) {
        // eslint-disable-next-line
        if (this.useEvent) return 'javascript:void(0);';
        var url = window.So360_Utils.urlAddQueryItem(`${this.baseUrl}&page=${num}`, 'qc', 'utf8');
        return this.currentPage == num ? 'javascript:void(0);' : url;
    },
    setBaseUrl() {
        if(this.baseUrl == '' || this.baseUrl==undefined) {
            var queryStr = window.So360_Utils.getBaseQueryStr();
    ...
    addparamsQc(url, key ,value){ // 链接添加 &qc=utf8 参数
        var param = window.So360_Utils.getQueryString(key);
